...
- All credentials are read from environment variables (e.g. `EBAY_CLIENT_ID`,
  `EBAY_CLIENT_SECRET`) or a local `.env` file.
...
1. **Rotate it immediately** at the provider — the old value is considered
   ...
   - For the marketplace API keyset: go to
     <https://developer.ebay.com> → **Application Keys**, and regenerate the
     **Cert ID (Client Secret)** for the affected keyset (delete the keyset and
     create a new one if needed). Update the env vars / CI secrets with the new
     value.
