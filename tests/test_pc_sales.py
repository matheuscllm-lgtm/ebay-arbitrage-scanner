"""PriceCharting — vendas concluídas (``src/pc_sales.py``), suíte OFFLINE.

Porte dos testes de ``scanner-comc/tests/test_pricecharting_sales.py``,
``test_pricecharting_client.py`` e ``test_review_fixes.py`` (@ dd952ba, 2026-09-03).
Cobre: parse de TODOS os baldes com dedupe por (source, sale_id) e janela limitada por
``</table>``; comparáveis exatos (certificadora + nota + subcategoria + variante);
vendas LP com referência própria (≥3, nunca thin); janelas 180 (ok) / 365 (low) /
1–2 (thin) / None; guardas de match nome/número/set; ``fetch_page`` com bloqueio →
``PcError`` sem cache, retry em 429/5xx/OSError e cache do dia em
``data/cache/pc/<AAAA-MM-DD>/``; ``graded_reference`` com ``column_key`` explícito
(sem módulo ``grading`` neste repo — ``grade`` é qualquer objeto com grader/value/
qualifier/label).

Fixtures reais (copiadas do scanner-comc): ``pc_product_charizard_base_4.html``
(Charizard 4/102 Base Set, capturada 2026-09-02: 375 vendas em 19 divs, 6 títulos LP,
16 PSA 8), ``pc_product_charizard_ex_151.html`` e ``pc_search_charizard_ex_151.html``.
``_today`` é congelado no dia da captura para as janelas de recência não envelhecerem.
Nenhum teste vai à rede: ``fetch_page``/``urlopen`` são sempre stubados.
"""
import dataclasses
...
from src.pc_sales import PcError, SalesRef
...
class _Grade:
    """Stand-in do ``Grade`` do scanner-comc: ``graded_reference`` só exige os atributos
    grader/value/qualifier/label (duck typing — não existe módulo ``grading`` aqui)."""
    grader: str
    value: float
    qualifier: str = ""
...
def no_sleep(monkeypatch):
    """Retry e educação com o site sem dormir: a suíte tem que ser rápida."""
    monkeypatch.setattr(pc, "REQUEST_GAP_SECONDS", 0.0)
    monkeypatch.setattr(pc, "RETRY_BACKOFF_SECONDS", 0.0)
...
def _sale_ago(days_ago, price, title="Charizard ex 006 BGS 9.5 GEM MINT"):
    return _sale(_day(days_ago), price, title)
...
# --- fetch_page: bloqueio nunca cacheado, retry transitório, cache do dia ---------------
...
def _http_error(code, msg="err"):
    return urllib.error.HTTPError("https://www.pricecharting.com/game/x/y", code, msg, {}, None)


GOOD_PAGE = ("<html><head><title>Charizard #4 Prices | Pokemon Base Set</title></head><body>"
             + "x" * 5000 + "</body></html>")


def test_fetch_page_never_caches_block_or_empty_pages(monkeypatch, tmp_path, no_sleep):
    ...
    assert not list(pc.today_cache_dir(tmp_path).glob("*.tmp"))
    ...
    monkeypatch.setattr(pc.urllib.request, "urlopen",
                        lambda req, timeout=30: calls.append(1) or _raise(urllib.error.URLError("dns")))
    with pytest.raises(PcError):
        pc.fetch_page("https://www.pricecharting.com/game/x/w", cache_dir=str(tmp_path))
    assert len(calls) == pc.FETCH_ATTEMPTS == 3  # esgotou as tentativas antes de subir
    assert not list(pc.today_cache_dir(tmp_path).glob("*"))


def test_fetch_page_retries_429_and_5xx_then_caches_good_page_for_the_day(monkeypatch, tmp_path, no_sleep):
    answers = [_http_error(503, "Service Unavailable"), _http_error(429, "Too Many Requests"), _Resp(GOOD_PAGE)]
    calls = []
    ...
    assert not list(pc.today_cache_dir(tmp_path).glob("*.tmp"))
    # 2ª chamada no mesmo dia = cache, sem rede
    assert pc.fetch_page(url, cache_dir=str(tmp_path)) == GOOD_PAGE and len(calls) == 3


def test_fetch_page_fails_fast_on_definitive_4xx(monkeypatch, tmp_path, no_sleep):
    calls = []
    monkeypatch.setattr(pc.urllib.request, "urlopen",
                        lambda req, timeout=30: calls.append(1) or _raise(_http_error(404, "Not Found")))
    with pytest.raises(PcError):
        pc.fetch_page("https://www.pricecharting.com/game/x/nao-existe", cache_dir=str(tmp_path))
    assert calls == [1]  # 4xx (fora 429) não é transitório: sem retry


def test_fetch_page_decodes_gzip_and_sends_browser_headers(monkeypatch, tmp_path, no_sleep):
    seen = {}

    def fake_urlopen(req, timeout=30):
        seen.update(req.headers)
        return _Resp(gzip.compress(GOOD_PAGE.encode()), headers={"Content-Encoding": "gzip"})

    monkeypatch.setattr(pc.urllib.request, "urlopen", fake_urlopen)
    assert pc.fetch_page("https://www.pricecharting.com/game/x/gz", cache_dir=str(tmp_path)) == GOOD_PAGE
    assert "Mozilla" in seen.get("User-agent", "") and seen.get("Accept-encoding") == "gzip"


def test_fetch_page_default_cache_is_data_cache_pc_scoped_by_day(monkeypatch, tmp_path, no_sleep):
...
def test_cache_dir_is_scoped_to_today(tmp_path):
    ...
    assert re.fullmatch(r"\d{4}-\d{2}-\d{2}", pc.today_stamp())
...
def test_parse_sales_same_id_from_another_source_is_a_different_sale():
    page = ('<div class="completed-auctions-used"><table>'
            + _row(7, "2026-08-01", "Charizard LP", 300.0, source="ebay")
            + _row(7, "2026-08-02", "Charizard LP", 310.0, source="tcgplayer")
            + '</table></div>')
    assert [(s["source"], s["sale_id"]) for s in pc.parse_sales(page)] == [("ebay", "7"), ("tcgplayer", "7")]
...
def test_parse_sales_stops_at_the_end_of_each_sales_table():
    """Linhas <tr id="ebay-N"> fora das tabelas de vendas (carrossel de itens parecidos
    no rodapé) nunca entram como venda da carta: a janela fecha no ``</table>``."""
...
def test_parse_sales_skips_rows_without_date_or_price():
    page = ('<div class="completed-auctions-used"><table>'
            '<tr id="ebay-1"><td class="title">sem data</td><td><span class="js-price">$10.00</span></td></tr>'
            '<tr id="ebay-2"><td class="date">2026-08-01</td><td class="title">sem preço</td></tr>'
            + _row(3, "2026-08-02", "Charizard LP", 30.0) + '</table></div>')
    assert [s["sale_id"] for s in pc.parse_sales(page)] == ["3"]
    assert pc.parse_sales("<html><body>nada</body></html>") == []
...
def test_comparable_sales_exact_grader_and_grade_only():
    ...
    assert [s["price"] for s in pc.comparable_sales(sales, "bgs", 9.5)] == [50.0, 55.0, 52.0]  # case
...
def test_title_citing_two_grades_is_excluded():
    sales = [
        _sale_ago(1, 500.0, "Charizard ex PSA 9 MINT (comps to PSA 10 sold $500!)"),
        _sale_ago(2, 20.0, "Charizard ex PSA 9 MINT"),
        _sale_ago(3, 0.0, "Charizard ex PSA 9 MINT"),  # preço zero não conta
    ]
    assert [s["price"] for s in pc.comparable_sales(sales, "PSA", 9.0)] == [20.0]
    assert pc.comparable_sales(sales, "PSA", 10.0) == []
    cgc = [_sale_ago(1, 60.0, "Charizard CGC 10 Pristine"), _sale_ago(2, 40.0, "Charizard CGC 10 Gem Mint"),
           _sale_ago(3, 45.0, "Charizard CGC 10 (flawless, pristine surface)")]
...
def test_bgs10_black_label_in_sale_titles_needs_label_context_not_card_name():
    """"Black Kyurem" tem "black" no NOME da carta — não é etiqueta preta.
    Só "Black Label"/"BGS 10 Black"/"Black BGS 10" identificam a subcategoria."""
    sales = [_sale("2026-08-01", 300.0, "2013 Black Kyurem EX Plasma Freeze BGS 10 PRISTINE"),
             ...
             _sale("2026-08-05", 880.0, "Black Kyurem EX black BGS 10")]
    assert [x["price"] for x in pc.comparable_sales(sales, "BGS", 10.0, "")] == [300.0, 320.0]
    assert [x["price"] for x in pc.comparable_sales(sales, "BGS", 10.0, "BLACK")] == [900.0, 850.0, 880.0]
...
def test_recent_only_drops_sales_older_than_180_days():
    sales = [_sale_ago(10, 50.0), _sale_ago(179, 55.0), _sale_ago(181, 60.0), _sale_ago(900, 65.0)]
    ...
    assert [s["price"] for s in pc.recent_only(sales, 365)] == [50.0, 55.0, 60.0]
    assert pc.recent_only([_sale("data-ruim", 1.0, "x")]) == []  # data inválida é descartada
...
def test_constants_match_operator_windows():
    ...
    assert pc.REQUEST_GAP_SECONDS == 2.0 and pc.MIN_PAGE_BYTES > 0
    assert pc.BASE_URL == "https://www.pricecharting.com"


def test_sales_ref_is_frozen():
    ref = SalesRef(price=1.0, n_sales=3, window_days=180, liquidity="ok", url="u", label="l")
...
def test_search_paths_accept_absolute_hrefs_and_dedupe():
    ...
    # href relativo e entidade HTML desescapada; href com query/fragmento não é resultado
    assert pc.search_card_paths('<a href="/game/pokemon-scarlet-&amp;-violet-151/charizard-ex-6">'
                                '<a href="/game/pokemon-base-set/charizard-4?x=1#y">') == \
        ["/game/pokemon-scarlet-&-violet-151/charizard-ex-6"]


def test_choose_path_prefers_english_console_matching_set_and_number():
    paths = pc.search_card_paths(SEARCH)
    assert pc.choose_path(paths, "Charizard ex", "6", SET_151) == \
        "/game/pokemon-scarlet-&-violet-151/charizard-ex-6"
    # set diferente -> None (nunca casa cross-set / outra língua)
    assert pc.choose_path(paths, "Charizard ex", "6", "SV: Paldean Fates") is None
    assert pc.choose_path(paths, "Charizard", "6", SET_151) is None  # falta 'ex'
    assert pc.choose_path([], "Charizard ex", "6", SET_151) is None


def test_choose_path_skips_reverse_slug_and_prefers_shortest():
    paths = ["/game/pokemon-base-set/charizard-holo-4", "/game/pokemon-base-set/charizard-4",
             "/game/pokemon-base-set/charizard-reverse-holo-4"]
    assert pc.choose_path(paths, "Charizard", "4/102", "Base Set") == "/game/pokemon-base-set/charizard-4"
    assert pc.choose_path(["/game/pokemon-base-set/charizard-reverse-holo-4"], "Charizard", "4", "Base Set") is None
...
def test_console_and_slug_guards():
    ...
    assert not pc.console_matches("/game/pokemon-base-set-2/charizard-4", "Base Set")  # bidirecional
    assert not pc.console_matches("/game/charizard-4", "Base Set")
    assert not pc.console_matches(p, "")
    assert pc.console_matches("/game/pokemon-emerald/charizard-4", "EX Emerald (em)")
...
def test_clean_card_and_set_names():
    assert pc.clean_card_name("Charizard ex - 006/165") == "Charizard ex"
    assert pc.clean_card_name("Charizard (Holo) Lv.76") == "Charizard"
    assert pc.clean_card_name(None) == ""
    assert pc.clean_set_name("SV: Scarlet & Violet 151") == "Scarlet & Violet 151"
    assert pc.clean_set_name("EX Emerald (em)") == "EX Emerald"
    assert pc.clean_set_name("Base Set") == "Base Set"
    assert pc.clean_set_name(None) == ""
...
def test_graded_reference_uses_sales_median_never_generic_bucket(monkeypatch, tmp_path):
    ...
    ref = pc.graded_reference("Charizard ex", "6", SET_151, _Grade("BGS", 9.5), cache_dir=str(tmp_path))
    assert isinstance(ref, SalesRef)
    assert (ref.price, ref.n_sales, ref.window_days, ref.liquidity) == (95.0, 4, 180, "ok")
    assert ref.label.startswith("vendas BGS 9.5 (n=4, ")
    assert ref.column_price is None  # BGS 9.5 não tem coluna própria (column_key=None)
...
def test_graded_reference_two_sales_is_thin_not_proxy(monkeypatch, tmp_path):
    ...
    ref = pc.graded_reference("Charizard ex", "6", SET_151, _Grade("TAG", 9.5), cache_dir=str(tmp_path))
...
def test_graded_reference_never_returns_column_without_sales(monkeypatch, tmp_path):
    ...
    assert pc.graded_reference("Charizard ex", "6", SET_151, _Grade("TAG", 10.0),
                               cache_dir=str(tmp_path), column_key="TAG 10") is None
...
def test_graded_reference_attaches_exact_column_as_information_only(monkeypatch, tmp_path):
    ...
    ref = pc.graded_reference("Charizard ex", "6", SET_151, _Grade("TAG", 10.0),
                              cache_dir=str(tmp_path), column_key="TAG 10")
    assert ref.price == 75.0 and ref.column_price == 77.0 and ref.liquidity == "ok"
    assert ref.label.startswith("vendas TAG 10 (n=3, ")


def test_graded_reference_column_key_none_means_no_column_price_even_if_column_exists(monkeypatch, tmp_path):
    """Diferença do porte: sem módulo ``grading``, a coluna exata vem só por ``column_key``.
    None → ``column_price`` None mesmo com a coluna "TAG 10" ($77) na página; chave que a
    página não tem → None; a referência (mediana de vendas) não muda em nenhum caso."""
    _stub_fetch(monkeypatch, _page_with_sales((70.0, 75.0, 80.0), label="TAG 10"))
    args = ("Charizard ex", "6", SET_151, _Grade("TAG", 10.0))
    ref = pc.graded_reference(*args, cache_dir=str(tmp_path))
    assert ref.price == 75.0 and ref.column_price is None
    ref2 = pc.graded_reference(*args, cache_dir=str(tmp_path), column_key="SGC 10")
    assert ref2.price == 75.0 and ref2.column_price is None
    ref3 = pc.graded_reference(*args, cache_dir=str(tmp_path), column_key="Grade 9.5")  # rótulo cru ≠ chave
    assert ref3.price == 75.0 and ref3.column_price is None


def test_graded_reference_accepts_any_grade_like_object(monkeypatch, tmp_path):
    _stub_fetch(monkeypatch, _page_with_sales((70.0, 75.0, 80.0), label="TAG 10"))
    grade = types.SimpleNamespace(grader="TAG", value=10.0, qualifier="", label="TAG 10")
    ref = pc.graded_reference("Charizard ex", "6", SET_151, grade, cache_dir=str(tmp_path), column_key="TAG 10")
    assert ref.price == 75.0 and ref.column_price == 77.0 and ref.label.startswith("vendas TAG 10 (n=3")
...
def test_graded_reference_black_label_needs_black_in_titles(monkeypatch, tmp_path):
    ...
    black = _Grade("BGS", 10.0, "BLACK")
    assert black.label == "BGS 10 Black Label"
    assert pc.graded_reference("Charizard ex", "6", SET_151, black, cache_dir=str(tmp_path),
                               column_key="BGS 10 BLACK") is None
    gold = pc.graded_reference("Charizard ex", "6", SET_151, _Grade("BGS", 10.0), cache_dir=str(tmp_path),
                               column_key="BGS 10")
    assert gold.price == 1450.0 and gold.column_price == 1600.0
    assert gold.label.startswith("vendas BGS 10 (n=3, ")


def test_graded_reference_cgc10_pristine_vs_gem(monkeypatch, tmp_path):
    page = _page_with_sales((60.0, 62.0, 64.0), label="CGC 10 Pristine",
                            columns='<tr><td>CGC 10</td><td>$40.00</td></tr>'
                                    '<tr><td>CGC 10 Pristine</td><td>$66.00</td></tr>')
    _stub_fetch(monkeypatch, page)
    pristine = pc.graded_reference("Charizard ex", "6", SET_151, _Grade("CGC", 10.0, "PRISTINE"),
                                   cache_dir=str(tmp_path), column_key="CGC 10 PRISTINE")
    assert pristine.price == 62.0 and pristine.column_price == 66.0
    assert pristine.label.startswith("vendas CGC 10 Pristine (n=3, ")
    # Gem Mint 10: as vendas "Pristine" não servem, mesmo com a coluna CGC 10 ($40) presente
    assert pc.graded_reference("Charizard ex", "6", SET_151, _Grade("CGC", 10.0, "GEM"),
                               cache_dir=str(tmp_path), column_key="CGC 10") is None
...
def test_graded_reference_passes_variants_to_comparables(monkeypatch, tmp_path):
    ...
    args = ("Charizard ex", "6", SET_151, _Grade("PSA", 9.0))
...
def test_graded_reference_end_to_end_with_stubbed_fetch(monkeypatch, tmp_path, frozen_today):
    calls = []
    ...
    args = ("Charizard ex", "6", SET_151)
    ref = pc.graded_reference(*args, _Grade("PSA", 10.0), cache_dir=str(tmp_path), column_key="PSA 10")
    assert isinstance(ref, SalesRef)
    ...
    assert ref.url.endswith("/game/pokemon-scarlet-&-violet-151/charizard-ex-6")
    assert len(calls) == 2
    assert calls[0].startswith("https://www.pricecharting.com/search-products?q=") and "type=prices" in calls[0]
    ref9 = pc.graded_reference(*args, _Grade("PSA", 9.0), cache_dir=str(tmp_path))
    ...
    assert pc.graded_reference(*args, _Grade("BGS", 9.5), cache_dir=str(tmp_path)) is None
...
def test_graded_reference_on_real_base_set_page(monkeypatch, tmp_path, frozen_today):
    ...
    psa8 = pc.graded_reference("Charizard", "4/102", "Base Set", _Grade("PSA", 8.0), cache_dir=str(tmp_path))
    ...
    psa10 = pc.graded_reference("Charizard", "4/102", "Base Set", _Grade("PSA", 10.0),
                                cache_dir=str(tmp_path), column_key="PSA 10")
    ...
    assert pc.graded_reference("Charizard", "4/102", "Base Set", _Grade("BGS", 10.0),
                               cache_dir=str(tmp_path), column_key="BGS 10") is None
    assert pc.graded_reference("Charizard", "4/102", "Base Set", _Grade("BGS", 10.0, "BLACK"),
                               cache_dir=str(tmp_path), column_key="BGS 10 BLACK") is None
...
def test_graded_reference_none_when_search_has_no_match(monkeypatch, tmp_path):
    ...
    assert pc.graded_reference("Charizard", "4/102", "Base Set", _Grade("PSA", 10.0),
...
def test_page_without_any_table_is_a_source_error_not_no_sales(monkeypatch, tmp_path):
    ...
    _stub_fetch(monkeypatch, page)
    with pytest.raises(PcError):
        pc.graded_reference("Charizard ex", "6", SET_151, _Grade("PSA", 10.0), cache_dir=str(tmp_path))


def test_graded_reference_propagates_pc_error_from_fetch(monkeypatch, tmp_path):
    monkeypatch.setattr(pc, "fetch_page", lambda url, cache_dir=None: _raise(PcError("bloqueio")))
    with pytest.raises(PcError):
        pc.graded_reference("Charizard ex", "6", SET_151, _Grade("PSA", 10.0), cache_dir=str(tmp_path))
    with pytest.raises(PcError):
        pc.product_page_url("Charizard ex", "6", SET_151, cache_dir=str(tmp_path))
...
def test_raw_condition_reference_requires_three_lp_sales_never_thin(monkeypatch, tmp_path):
    _stub_fetch(monkeypatch, _page_with_sales((300.0, 320.0), label="Holo LP"))
    assert pc.raw_condition_reference("Charizard ex", "6", SET_151, cache_dir=str(tmp_path)) is None
    _stub_fetch(monkeypatch, _page_with_sales((300.0, 320.0, 310.0), label="Holo LP"))
    ref = pc.raw_condition_reference("Charizard ex", "6", SET_151, cache_dir=str(tmp_path))
    assert (ref.price, ref.n_sales, ref.liquidity) == (310.0, 3, "ok")
    assert ref.label.startswith("vendas LP (n=3, ")
...
def test_product_page_none_without_match_and_pc_error_without_tables(monkeypatch, tmp_path):
    ...
    with pytest.raises(PcError):
        pc._product_page("Charizard ex", "6", SET_151, str(tmp_path))
    with pytest.raises(PcError):
        pc.raw_condition_reference("Charizard ex", "6", SET_151, cache_dir=str(tmp_path))
    _stub_fetch(monkeypatch, PRODUCT)
    url, page = pc._product_page("Charizard ex", "6", SET_151, str(tmp_path))
...
def test_product_page_url_resolves_exact_card_page(monkeypatch, tmp_path):
    """Link [referência]: a página exata da carta no PriceCharting (mesmo match
    nome+número+set dos slabs); sem match → None; nunca inventa URL."""
    _stub_fetch(monkeypatch, PRODUCT)
    url = pc.product_page_url("Charizard ex", "006/165", SET_151, cache_dir=str(tmp_path))
    assert url == "https://www.pricecharting.com/game/pokemon-scarlet-&-violet-151/charizard-ex-6"
    assert pc.product_page_url("Cartanaoexiste", "999/165", SET_151, cache_dir=str(tmp_path)) is None
    assert pc._product_path("Charizard ex", "006/165", SET_151, cache_dir=str(tmp_path)) == \
        "/game/pokemon-scarlet-&-violet-151/charizard-ex-6"
...
def test_product_page_url_tolerates_page_without_tables(monkeypatch, tmp_path):
    ...
    um link válido — não é erro de fonte. Só a busca é consultada (1 chamada)."""
    calls = []

    def fake_fetch(url, cache_dir=None):
        calls.append(url)
        return SEARCH if "search-products" in url else "<html><body>" + "layout sem tabelas " * 400 + "</body></html>"
